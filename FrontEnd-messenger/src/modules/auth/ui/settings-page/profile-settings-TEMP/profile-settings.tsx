...
export const ProfileSettings: React.FC<{ authToken: string }> = ({
  authToken,
}) => {
  const {
    control,
    ...
    const fetchProfile = async () => {
      try {
        ...
        if (data.status === "ok") {
          ...
          const initialBirthday = data.data.birthday ? new Date(data.data.birthday) : null;
  ...
  const handleDateChange = (event: any, selectedDate?: Date) => {
    ...
    setValue("birthday", currentDate, { shouldDirty: true });
  };
  ...
  const handleFormSubmit = async (data: FormData) => {
    ...
    try {
      ...
      const response = await fetch("http://172.20.10.7:5000/api/usernative/profile", {
        method: "PUT",
        headers: {
      ...
      setProfileData((prev) => ({
        ...
        email: data.email, // обновляем email
